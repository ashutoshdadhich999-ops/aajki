...
for fair audio denoising comparisons.
"""
...
def evaluate_audio(
    ...
    print(
        ...
        f"[{corruption}] "
        f"MSE: {res['MSE']:.5f} | "
        f"SI-SDR Imp: {res['SI-SDR Imp']:.2f} dB | "
...
def measure_time(
    ...
    t_s, t_s_std = _time_one(
        model_spiking
    )

    t_ns, t_ns_std = _time_one(
        model_nonspiking
    )
